...
- **Critical, non-obvious**: `front/src/lib/wasm.ts` imports the
  `flp-extract-fxp` package, which `front/package.json` links to the
  gitignored repo-root `pkg/` directory (`"link:../pkg"`). That `pkg/`
  directory is **not checked in** — it must be generated with `wasm-pack`
  before `pnpm dev`/`pnpm build` will even typecheck:
  ```sh
...
- From `front/`: `pnpm install`, `pnpm dev`, `pnpm build` (= `tsc -b && vite
  build`, needs the root `pkg/` to exist first), `pnpm preview`. There is no
