...
//! - [`flpconv`]: FLP byte surgery rewriting Serum 1 event-213 payloads
//!   into Serum 2 ones (plan / bundle / apply pipeline).
//! - [`importer`]: Serum 1 → Serum 2 conversion (a faithful port of
//!   Serum2.vst3's `s1state_load`; split into `params`, `modmatrix`,
//!   `fxrack`, `lfo`, `meta` submodules).
//! - [`s1state`]: Serum 1 preset-state parsing (typed view of the
//!   172,736-byte state blob).
//! - [`s2tables`]: GENERATED runtime-dumped Serum 2 descriptor tables
//!   (never hand-edit; provenance in docs/s2-runtime-tables.md).
//! - [`s2tree`]: deterministic CBOR value tree + raw zstd frames.
//! - [`serum2state`]: Serum 2 `XferJson` container assembly and parsing.
