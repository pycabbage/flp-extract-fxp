# FLP: PluginParams structure for Serum 1 vs Serum 2 (real-sample calibration)
...
samples), calibrated against a real project containing both a Serum 1 and a
Serum 2 VST3 instance.

All observations below come from the sample `assets/serina flps/serina1/serina1.flp`
(5 Serum 1 instances + 1 genuine Serum 2 instance + 40 other plugin instances),
cross-checked against `serina4.flp` (7 × Serum 1, 1 × Serum 2) and
`serina3/Project_7.flp` (2 × Serum 1, no Serum 2). Scripts/logs used for the
calibration are not in the repo; numbers here are transcribed from those dumps.

Related docs: `serum-fxp-format.md` (Serum 1 fxp), `serum2-importer-analysis.md`
...
Record order is fixed and identical for Serum 1 and Serum 2 (and every other
...
### 2.1 Record inventory — Serum 1 instance #0 (stream offset 0x2E6B)
...
### 2.2 Record inventory — Serum 2 instance (stream offset 0xE69C)
...
| 2   | runtime record | 25     | see §2.4                                                       |
| 30  | ?              | 16     | identical to Serum 1                                           |
| 32  | ?              | 12     | identical to Serum 1                                           |
| 50  | ?              | 16     | identical to Serum 1                                           |
...
| 56  | vendor         | 12     | identical to Serum 1 (`Xfer Records`)                          |
...
except the Serum 2 instances, where `A = 1`. The value is stable per plugin
...
to mirror a genuine Serum 2 instance byte-for-byte should write `01 00 00 00`
...
```
Serum 1 : 58 54 53 56 73 66 73 58 65 72 75 6D 00 00 00 00   "XTSVsfsXerum"
Serum 2 : 58 45 53 56 73 66 73 50 65 72 75 6D 20 32 00 00   "XESVsfsPerum 2"
...
Serum 2 (and iZotope plugins — Ozone Imager 2 also saves `[1, 3, 2, 4]` with
`size(cid2) == size(cid3)` = 1415) and absent for Serum 1, whose controller
...
samples (Serum 1, Serum 2, Vital, Kilohearts, OTT, iZotope...). Treat it as a
...
### 3.3 Serum 1 inner cid 3 — zlib streams + u32 LE trailer
...
- Serum 1: count = 0x95D = **2,397**; list ≈ `0,1,2,...,2396` with a handful of
  vendor-range ids (e.g. `707Fxxxx`-class values around entry 315).
- Serum 2: count = 0xA3F = **2,623**; list starts `0..8`, skips 9, continues
...
it from a genuine Serum 2 state when converting (do not try to reuse S1's).

## 4. Serum 2 XferJson records (inner cid 3 = processor, inner cid 2 = controller)
...
§"Serum 2"): 7-bit-ASCII length-prefixed keys/values, marker bytes `A0`/`A1`,
...
**Serum 2 preset identity lives inside the cid-2 XferJson record
(JSON `presetName` + decompressed controller body), not in any separate FLP
event.** For Serum 1 the equivalent lives inside the zlib stream at offset
0x4972 (see §3.3).
...
| 203 | 0x7236 | `Serum #2` |
| 203 | 0xE67E | `Serum 2`   ← the real Serum 2 channel |
| 203 | 0x20B88 | `Serum #3` |
| 203 | 0x25CF9 | `Serum #4` |
| 203 | 0x2B873 | `Serum #5` |
...
- These are pure display labels: they are user-editable, independent of the
  loaded plugin (the S1 channels are named `Serum #N`, the Serum 2 channel
...
**Conclusion: swapping Serum 1 → Serum 2 requires changing only the event-213
payload (plus lengths). No other events must change.**

## 6. Rewrite recipe: Serum 1 event 213 → Serum 2 event 213

Given the S1 instance's 213 payload and a genuine Serum 2 XferJson state
(ideally lifted from a real Serum 2 instance in some FLP, as done for this
doc), the deltas are:
...
| # | where | from | to |
|---|-------|------|----|
| 1 | top cid 1, byte offset 8 (u32 LE) | `0C 00 00 00` | `01 00 00 00` (real Serum 2 value; semantics unknown, §2.3) |
...
| 5 | top cid 53, record size (u64 LE) | 16,257 | size of the new Serum 2 wrapper state |
...
| 9 | inner cid 4 | S1's 2,397-entry list | Serum 2's list (2,623 entries for 2.0.22) — copy from a genuine Serum 2 instance, do not synthesize |
...
| file | FLdt compressed | trailing bytes | events | event-213 count | Serum 1 | Serum 2 |
|------|-----------------|----------------|--------|-----------------|---------|---------|
...
serina1 counts (5 × Serum 1 + 1 × Serum 2) agree with the extraction CLI's
report. Serum 1 instance stream offsets in serina1.flp: `0x2E6B, 0x7256,
0x20BA8, 0x25D19, 0x2B893`; the Serum 2 instance: `0xE69C`. The one real Serum
2 instance sits among 40 other VST3/VST2 plugin instances; its plugin state
