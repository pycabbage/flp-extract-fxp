...
flp-extract-fxp extract --name-template "{preset}_{author}" a.flp
```
...
| `--name-template <TPL>` | 出力ファイル名テンプレート。既定は `{index}_{preset}` (現行の命名) |
...
未知のプレースホルダと空の値は空文字に置き換わり、展開結果が空になった場合は現行のフォールバック (プリセット名 → チャンネル名 → `Serum N`) を使います。サニタイズはテンプレート全体ではなく**置換される値ごと**に適用されるため、プリセット名に `/` などが含まれてもパス注入は起こりません (テンプレート文字列自体にリテラルで `/` を入れた場合はその限りではありません)。同一ベース名は `名_2.fxp` のように接尾番号を付けます。
